...
class DelegatableAuth:
    ...
    def export_revocations(self) -> frozenset[str]:
        """Export revoked segment ids.

        Example::

            revoked = auth.export_revocations()
        """
        return frozenset(self._revoked)

    def import_revocations(self, revoked_ids: set[str] | frozenset[str]) -> None:
        """Import revoked segment ids.

        Example::

            auth.import_revocations(previously_exported_ids)
        """
...
def _validate_child_scopes(child_scopes: list[str], parent_scopes: list[str]) -> None:
    for scope in [*parent_scopes, *child_scopes]:
        _validate_efs_write_scope(scope)
    if child_scopes == _scopes(parent_scopes):
        msg = "Child scopes must be a strict subset of parent scopes"
